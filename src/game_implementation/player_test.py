...
class TestPlayer:
    ...
    def test_init_with_init(self):
        player = Player(0, [DiscState.Vulnerable, DiscState.Safe, DiscState.Gone,] * 2, [], 0)
    ...
    def test_reset(self):
        player = Player(
            0, [DiscState.Vulnerable, DiscState.Gone, DiscState.Safe] + [DiscState.Vulnerable] * 3, [3, 4], 1
        )
    ...
    def test_end_round(self, disks: List[DiscState], taken: List[int], current_score: int, expected_score: int):
        player = Player(0, disks, taken, current_score)
    ...
    def test_make_safe(self, disks: List[DiscState], safe_disk_id: int, expected_disks: Union[bool, List[DiscState]]):
        player = Player(0, disks, [], 0)

        if expected_disks:
            player.make_safe(safe_disk_id)
            assert player.discs == expected_disks
        else:
            with pytest.raises(Exception, match=f"cannot make safe {player.discs[safe_disk_id]}"):
                player.make_safe(safe_disk_id)
    ...
    def test_make_vulnerable(
        ...
        if expected_disks:
            ...
            with pytest.raises(Exception, match=f"cannot make vulnerable {player.discs[safe_disk_id]}"):
                player.make_vulnerable(safe_disk_id)
    ...
    def test_make_gone(self, disks: List[DiscState], safe_disk_id: int, expected_disks: Union[bool, List[DiscState]]):
        ...
        if expected_disks:
            ...
            with pytest.raises(Exception, match=f"cannot take {player.discs[safe_disk_id]}"):
                player.make_gone(safe_disk_id)
